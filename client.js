...
function request(data, endpoint) {
    ...
    var old_data = data;
    // check if add row .
    if (endpoint === "/client/addrow" && ip_to_send === MASTER_IP)
    ...
    $.post("http://" + ip_to_send + new_endpoint, old_data, function(response) {
        ...
        if (obj.tablet_IP != null) {
            ...
            $.post("http://" + ip_to_send + endpoint, data, function(response) {

                var obj = JSON.parse(response);
                console.log(" back fromtablet with ip.. :" + ip_to_send);
                results_html.empty();
                results_html.append("back from tablet with ip : " + ip_to_send + "<br>");
